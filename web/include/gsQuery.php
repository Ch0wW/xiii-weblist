 ...
 * @version $Id: gsQuery.php,v 1.26 2004/05/23 12:24:00 jr Exp $
 *
...
class gsQuery
{
  ...
  {
    $this->version = mb_ereg_replace("[^0-9\\.]", "", "\$Revision: 1.26 $");
  ...
  function createInstance($protocol, $address, $port) {
    ...
    default:
      ...
      if(mb_ereg("^[A-Za-z0-9_-]+$", $protocol) && mb_ereg("^[A-Za-z0-9\\.-]+$", $address) && is_numeric($port)) {
  ...
  function unserialize($string) 
  {
    ...
    // we should be careful when using eval with supplied arguments
    if(mb_ereg("^[A-Za-z0-9_-]+$", $className)) {
  ...
  function getSupportedProtocols($path) 
  {
    ...
    while(false!==($curfile=readdir($handle))) {
      if($curfile!="gsQuery.php" && $curfile!="index.php" && mb_ereg("^(.*)\.php$", $curfile, $matches)) {
  ...
  function serialize() 
  {   
    return get_class($this) .":". base64_encode(serialize($this));
  }
